...
import {
  getSaintInfoList, getDeptTypeList, getVillageTypeList, registerSaint, getSaintAttList
} from "../../api/Api";
...
class Attendance extends Component {
  ...
  handleResetSearchOption(){
    ...
    var villageNameElement = document.getElementById("villageName");
    var saintNameElement = document.getElementById("saintName");
    var attDateElement = document.getElementById("attDate");
  }
  ...
  handleChangeAttDate(event) {
    console.log(event.target.value)
    this.setState({
      attDate: event.target.value
    })
  }
  ...
  retrieveSaintAttList() {
    var searchDate = this.state.attDate;
    // if (null == searchDate) {
    ...
    const saintRequest = {
      ...
      saintName: this.state.saintName,
      villageName: this.state.villageName
    }

    getSaintAttList(saintRequest)
    .then(res => {
      const result = res.status;
  ...
  render() {
    ...
    return (
          ...
          <GridItem xs={12} sm={12} md={12}>
            <Card>
              <CardHeader color="primary">
                <h3 className={classes.cardTitleWhite}>출석부 검색</h3>
      ...
      <TextField
        ...
        label="출석날"
        type="date"
        onChange={(e) => this.handleChangeAttDate(e)}
      ...
      />
                  ...
                  <Button 
                    variant="contained" 
                    color="primary"
                    onClick={() => this.retrieveSaintAttList()}
                    >
        조회
      </Button>
      </div>
                ...
                <OriginalButton 
                    variant="outlined" 
                    color="secondary"
                    onClick={() => this.handleResetSearchOption()}
                    >
        초기화
      </OriginalButton>
