...
export function registerSaint(SaintRequest) {
    const body = {
        dept: SaintRequest.registerDept,
        village: SaintRequest.registerVillage,
        age: SaintRequest.registerAge,
        name: SaintRequest.registerSaintName,
        preChurch: SaintRequest.registerPreChurch,
        gender: SaintRequest.registerGender,
        major: SaintRequest.registerMajor,
        address: SaintRequest.registerAddress,
        birthday: SaintRequest.registerBirthday,
        baptism: SaintRequest.registerBaptism,
        discipleTraining: SaintRequest.registerDiscipleTraining
    };

    const path = base_url + "/saint/registerSaint";

    return postMethod(path, body, postHeader);
}

export function getDeptTypeList() {
    const path = base_url + "/dept/getDeptTypeList";

    return getMethod(path, getHeader);
}

export function getVillageTypeList() {
    const path = base_url + "/village/getVillageTypeList";
...
export function getSaintInfoList(saintName, villageName) {
    const body = {
        saintName: saintName,
